#!/usr/bin/env python3
"""
Automated karaoke-style subtitler for Russian songs
–– now powered by GigaAM (instead of Parakeet) for Russian ASR + word-level timestamps.
The rest of the pipeline (vocal isolation, translation, subtitle burning) is unchanged.

Modifications:
- Extract audio as WAV directly (no intermediate OPUS).
- Optional vocal isolation controlled by a 3rd CLI argument:
    use_vocal_isolation = 0 (default) or 1.
"""
...
# -------------  GigaAM long-form wrapper  -------------
import gigaam_longform as gigaam_lf
# ------------------------------------------------------
...
def extract_audio(video_path, audio_output_path, target_sr=16000):
    """
    Extract audio from video directly as WAV at target_sr (mono).
    """
    ...
    try:
        ...
        print(f"✓ Audio extracted as WAV ({target_sr} Hz, mono).")
...
def main():
    ...
    parser.add_argument(
        ...
        nargs="?",
        default="1",
        help="Optional: 0 = no vocal isolation (default), 1 = use vocal isolation",
    )
    ...
    # Parse vocal isolation flag
    try:
        use_vocal_isolation = int(args.use_vocal_isolation)
    except ValueError:
        use_vocal_isolation = 1
    if use_vocal_isolation not in (0, 1):
        print(f"Invalid use_vocal_isolation '{args.use_vocal_isolation}', defaulting to 1 (vocal isolation).")
        use_vocal_isolation = 1

    # Variables
    video_file = f"{project_name}.mp4"
    input_audio_wav = f"{project_name}.wav"    # extracted audio
    ...
    VOCAL_MODEL_FILENAME = 'vocals_mel_band_roformer.ckpt'
    bnb_config = BitsAndBytesConfig(load_in_8bit=True, llm_int8_threshold=6.0)

    # --- SETUP ---
    install_fonts()
    download_video(url, project_name)
    extract_audio(video_file, input_audio_wav, target_sr=16000)

    ###############################################################
    ###                     VOCAL ISOLATION (OPTIONAL)          ###
    ###############################################################
    if use_vocal_isolation == 1:
        ...
        vocal_audio_files = separator.separate(input_audio_wav, output_names)
        ...
        # Clean up separator to free GPU memory
        del separator
        torch.cuda.empty_cache()
        gc.collect()

        asr_input_path = vocal_path
    else:
        print("\n=== Step 1: Skipping vocal isolation (using extracted WAV directly) ===")
        # Use the extracted WAV directly for transcription
        asr_input_path = input_audio_wav

    ###############################################################
    ###                     TRANSCRIPTION                       ###
    ###############################################################
    print("\n=== Step 2: Transcription (GigaAM) ===")

    # --- load GigaAM once ---
    device = "cuda" if torch.cuda.is_available() else "cpu"
    gigaam_model = gigaam_lf.gigaam.load_model("v3_e2e_rnnt", device=device)

    # --- run GigaAM long/short auto switch ---
    result = gigaam_lf.transcribe_auto(gigaam_model, asr_input_path)   # -> {words, transcript, ...}

    # --- reshape to the list-of-dicts format the rest of the script expects ---
    word_timestamps = [
    ...
    # Thresholds for timing-based rules
    MAX_SENTENCE_LENGTH = 12              # Split on comma if sentence would exceed this many words
    CAPITAL_SPLIT_SILENCE = 1.2          # Silence threshold for capital letter splits (sec)
    VERY_LONG_COMMA_SILENCE_SEC = 1.2    # Very long pause after comma triggers split

    def simple_sentence_chunking(word_timestamps):
        """
        Sentence chunking with strict rules:

        RULE 1: Always split on hard stops (. ! ? ; etc) and at end of text.
        RULE 2: Split on comma if NOT splitting would result in a sentence > MAX_SENTENCE_LENGTH words.
        RULE 3: Split when the NEXT word starts with a capital letter AND
                the silence before it is >= CAPITAL_SPLIT_SILENCE (regardless of punctuation).
        RULE 4: Split on comma if the silence AFTER the comma is >= VERY_LONG_COMMA_SILENCE_SEC.
        RULE 5: Split before a capital letter if NOT splitting would result in a sentence > MAX_SENTENCE_LENGTH words.
        """
        ...
        for i, stamp in enumerate(word_timestamps):
            ...
            current_len = len(current_sentence_words)
            ...
            # RULE 1: Always break at last word or hard stop
            if is_last_word or has_hard_stop:
                should_break = True

            # Check conditions for next word (for RULE 2, 3, 4, and 5)
            if not should_break and i < n - 1:
                next_stamp = word_timestamps[i + 1]
                gap = next_stamp['start'] - end
                next_word = next_stamp['word']
                starts_capital = bool(next_word and next_word[0].isupper())

                # RULE 3: Break on capital letter with sufficient preceding silence
                # This applies REGARDLESS of punctuation (comma or not)
                if starts_capital and gap >= CAPITAL_SPLIT_SILENCE:
                    should_break = True

                # RULE 2 & 4: Comma-based splits (only if Rule 3 didn't trigger)
                elif has_comma:
                    # RULE 2: Break on comma if continuing would make sentence too long
                    # Count words until next break point (hard stop, comma, or capital+silence)
                    projected_length = current_len
                    for j in range(i + 1, n):
                        future_stamp = word_timestamps[j]
                        future_word = future_stamp['word']

                        # Stop projecting at hard stops or commas
                        if any(future_word.endswith(stop) for stop in HARD_STOPS):
                            projected_length += 1
                            break
                        if any(future_word.endswith(comma) for comma in COMMA):
                            projected_length += 1
                            break

                        # Stop projecting at capital + silence
                        if j > i + 1:
                            prev_future = word_timestamps[j - 1]
                            future_gap = future_stamp['start'] - prev_future['end']
                            if future_word[0].isupper() and future_gap >= CAPITAL_SPLIT_SILENCE:
                                break

                        projected_length += 1

                        # Limit projection distance to avoid expensive computation
                        if j - i > 20:
                            break

                    if projected_length > MAX_SENTENCE_LENGTH:
                        should_break = True

                    # RULE 4: Break on comma if followed by very long silence
                    if not should_break and gap >= VERY_LONG_COMMA_SILENCE_SEC:
                        should_break = True

                # RULE 5: Break before capital letter if continuing would make sentence too long
                elif starts_capital:
                    # Count words until next break point
                    projected_length = current_len
                    for j in range(i + 1, n):
                        future_stamp = word_timestamps[j]
                        future_word = future_stamp['word']
    ...
    sentences_data = simple_sentence_chunking(word_timestamps)
